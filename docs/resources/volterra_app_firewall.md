...
`disable_staging` - (Optional) Enforce new and modified attack signatures. (bool).

`stage_new_and_updated_signatures` - (Optional) false positive detections.. See [Stage New And Updated Signatures ](#stage-new-and-updated-signatures) below for details.
...
`staging_period` - (Required) The default staging period is 7 days. (`Int`).
