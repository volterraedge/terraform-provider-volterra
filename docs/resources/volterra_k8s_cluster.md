...
`cluster_wide_app_list` - (Optional) Select K8s cluster role bindings for this K8s cluster. See [Cluster Wide App List ](#cluster-wide-app-list) below for details.

`no_cluster_wide_apps` - (Optional) There are o cluster wide applications to be deployed (bool).
...
`dashboard` - (Optional) Deploy Argo Continuous Deployment(CD) application. See [Dashboard ](#dashboard) below for details.
