    ...
    user_behavior_analysis_setting {
      ...
      enable_detection {
        ...
        include_failed_login_activity {
          login_failures_threshold = "login_failures_threshold"
        }
...
`metric` - (Optional) Metric enabled to be analyzed (`List of Strings`).

`metrics_source` - (Optional) Sources from where this metric is gathered for analyses (`String`).
