...
func resourceVolterraServicePolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	//simple_rules
	if v, ok := d.GetOk("simple_rules"); ok && !isIntfNil(v) {

		sl := v.([]interface{})
		simpleRules := make([]*ves_io_schema_service_policy.SimpleRule, len(sl))
		createSpec.SimpleRules = simpleRules
		for i, set := range sl {
			simpleRules[i] = &ves_io_schema_service_policy.SimpleRule{}
			simpleRulesMapStrToI := set.(map[string]interface{})

			if v, ok := simpleRulesMapStrToI["action"]; ok && !isIntfNil(v) {

				simpleRules[i].Action = ves_io_schema_policy.RuleAction(ves_io_schema_policy.RuleAction_value[v.(string)])

			}

			if v, ok := simpleRulesMapStrToI["api_group_matcher"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				apiGroupMatcher := &ves_io_schema_policy.StringMatcherType{}
				simpleRules[i].ApiGroupMatcher = apiGroupMatcher
				for _, set := range sl {
					apiGroupMatcherMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["asn_list"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				asnList := &ves_io_schema_policy.AsnMatchList{}
				simpleRules[i].AsnList = asnList
				for _, set := range sl {
					asnListMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["client_role"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				clientRole := &ves_io_schema_policy.RoleMatcherType{}
				simpleRules[i].ClientRole = clientRole
				for _, set := range sl {
					clientRoleMapStrToI := set.(map[string]interface{})
			...
			if w, ok := simpleRulesMapStrToI["description"]; ok && !isIntfNil(w) {
				simpleRules[i].Description = w.(string)
			}

			if v, ok := simpleRulesMapStrToI["domain_matcher"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				domainMatcher := &ves_io_schema_policy.MatcherType{}
				simpleRules[i].DomainMatcher = domainMatcher
				for _, set := range sl {
					domainMatcherMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["dst_ip_prefix_list"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				dstIpPrefixList := &ves_io_schema_policy.PrefixMatchList{}
				simpleRules[i].DstIpPrefixList = dstIpPrefixList
				for _, set := range sl {
					dstIpPrefixListMapStrToI := set.(map[string]interface{})

					if w, ok := dstIpPrefixListMapStrToI["invert_match"]; ok && !isIntfNil(w) {
						dstIpPrefixList.InvertMatch = w.(bool)
					}

					if w, ok := dstIpPrefixListMapStrToI["ip_prefixes"]; ok && !isIntfNil(w) {
						ls := make([]string, len(w.([]interface{})))
						for i, v := range w.([]interface{}) {
							ls[i] = v.(string)
						}
						dstIpPrefixList.IpPrefixes = ls
					}
			...
			if w, ok := simpleRulesMapStrToI["expiration_timestamp"]; ok && !isIntfNil(w) {
				ts, err := parseTime(w.(string))
				if err != nil {
					return fmt.Errorf("error creating ExpirationTimestamp, timestamp format is wrong: %s", err)
				}
				simpleRules[i].ExpirationTimestamp = ts
			}

			if v, ok := simpleRulesMapStrToI["headers"]; ok && !isIntfNil(v) {

				sl := v.([]interface{})
				headers := make([]*ves_io_schema_policy.HeaderMatcherType, len(sl))
				simpleRules[i].Headers = headers
				for i, set := range sl {
					headers[i] = &ves_io_schema_policy.HeaderMatcherType{}
					headersMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["http_method"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				httpMethod := &ves_io_schema_policy.HttpMethodMatcherType{}
				simpleRules[i].HttpMethod = httpMethod
				for _, set := range sl {
					httpMethodMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["ip_prefix_list"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				ipPrefixList := &ves_io_schema_policy.PrefixMatchList{}
				simpleRules[i].IpPrefixList = ipPrefixList
				for _, set := range sl {
			...
			if v, ok := simpleRulesMapStrToI["l4_dest_matcher"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				l4DestMatcher := &ves_io_schema_policy.L4DestMatcherType{}
				simpleRules[i].L4DestMatcher = l4DestMatcher
				for _, set := range sl {
					l4DestMatcherMapStrToI := set.(map[string]interface{})
			...
			if w, ok := simpleRulesMapStrToI["metric_name_label"]; ok && !isIntfNil(w) {
				simpleRules[i].MetricNameLabel = w.(string)
			}

			if w, ok := simpleRulesMapStrToI["name"]; ok && !isIntfNil(w) {
				simpleRules[i].Name = w.(string)
			}

			if v, ok := simpleRulesMapStrToI["path"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				path := &ves_io_schema_policy.PathMatcherType{}
				simpleRules[i].Path = path
				for _, set := range sl {
					pathMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["port_matcher"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				portMatcher := &ves_io_schema_policy.PortMatcherType{}
				simpleRules[i].PortMatcher = portMatcher
				for _, set := range sl {
					portMatcherMapStrToI := set.(map[string]interface{})
			...
			if w, ok := simpleRulesMapStrToI["scheme"]; ok && !isIntfNil(w) {
				ls := make([]string, len(w.([]interface{})))
				for i, v := range w.([]interface{}) {
					ls[i] = v.(string)
				}
				simpleRules[i].Scheme = ls
			}

			if v, ok := simpleRulesMapStrToI["tls_fingerprint_matcher"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				tlsFingerprintMatcher := &ves_io_schema_policy.TlsFingerprintMatcherType{}
				simpleRules[i].TlsFingerprintMatcher = tlsFingerprintMatcher
				for _, set := range sl {
					tlsFingerprintMatcherMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["url_matcher"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				urlMatcher := &ves_io_schema_policy.URLMatcherType{}
				simpleRules[i].UrlMatcher = urlMatcher
				for _, set := range sl {
					urlMatcherMapStrToI := set.(map[string]interface{})
			...
			if v, ok := simpleRulesMapStrToI["waf_action"]; ok && !isIntfNil(v) {

				sl := v.(*schema.Set).List()
				wafAction := &ves_io_schema_policy.WafAction{}
				simpleRules[i].WafAction = wafAction
				for _, set := range sl {
					wafActionMapStrToI := set.(map[string]interface{})
...
func resourceVolterraServicePolicyUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("simple_rules"); ok && !isIntfNil(v) {
		...
		sl := v.([]interface{})
		simpleRules := make([]*ves_io_schema_service_policy.SimpleRule, len(sl))
		updateSpec.SimpleRules = simpleRules
		for i, set := range sl {
			simpleRules[i] = &ves_io_schema_service_policy.SimpleRule{}
			simpleRulesMapStrToI := set.(map[string]interface{})
