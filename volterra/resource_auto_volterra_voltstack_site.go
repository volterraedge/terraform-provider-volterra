...
import (
	...
	ves_io_schema_views_voltstack_site "gopkg.volterra.us/terraform-provider-volterra/pbgo/extschema/schema/views/voltstack_site"
...
func resourceVolterraVoltstackSiteCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("k8s_cluster"); ok && !isIntfNil(v) && !k8SClusterChoiceTypeFound {
	...
	if v, ok := d.GetOk("local_control_plane"); ok && !isIntfNil(v) && !localControlPlaneChoiceTypeFound {
	...
	if v, ok := d.GetOk("master_nodes"); ok && !isIntfNil(v) {
		...
		for i, v := range v.([]interface{}) {
			if v == nil {
				return fmt.Errorf("please provide valid non-empty string value of field master_nodes")
			}
			if str, ok := v.(string); ok {
				ls[i] = str
			}
	...
	if v, ok := d.GetOk("custom_storage_config"); ok && !isIntfNil(v) && !storageCfgChoiceTypeFound {
...
func resourceVolterraVoltstackSiteDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return client.DeleteObject(context.Background(), ves_io_schema_views_voltstack_site.ObjectType, namespace, name, opts...)
}
