...
func (c *customAPIInprocClient) VerifyDnsDomain(ctx context.Context, in *VerifyDnsDomainRequest, opts ...grpc.CallOption) (*VerifyDnsDomainResponse, error) {
	ctx = server.ContextWithRpcFQN(ctx, "ves.io.schema.dns_domain.CustomAPI.VerifyDnsDomain")
...
func (s *customAPISrv) VerifyDnsDomain(ctx context.Context, in *VerifyDnsDomainRequest) (*VerifyDnsDomainResponse, error) {
	ah := s.svc.GetAPIHandler("ves.io.schema.dns_domain.CustomAPI")
	...
	bodyFields := svcfw.GenAuditReqBodyFields(ctx, s.svc, "ves.io.schema.dns_domain.VerifyDnsDomainRequest", in)
	defer func() {
		if len(bodyFields) > 0 {
	...
	if s.svc.Config().EnableAPIValidation {
		if rvFn := s.svc.GetRPCValidator("ves.io.schema.dns_domain.CustomAPI.VerifyDnsDomain"); rvFn != nil {
	...
	bodyFields = append(bodyFields, svcfw.GenAuditRspBodyFields(ctx, s.svc, "ves.io.schema.dns_domain.VerifyDnsDomainResponse", rsp)...)
