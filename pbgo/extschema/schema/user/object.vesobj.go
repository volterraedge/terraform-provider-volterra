...
const (
	ObjectDefTblName = "ves.io.schema.user.Object.default"
	ObjectType       = "ves.io.schema.user.Object"

	StatusObjectDefTblName = "ves.io.schema.user.StatusObject.default"
	StatusObjectType       = "ves.io.schema.user.StatusObject"
)

// augmented methods on protoc/std generated struct
func (e *Object) Type() string {
	return "ves.io.schema.user.Object"
}
...
func GetObjectIndexers() store.Indexers {
	indexers := store.Indexers{
		"metadata.name": store.NewIndexInfo(store.WithSecondaryIndex(func(e store.Entry) ([]string, error) {
			obj, ok := e.(*Object)
			if !ok {
				return nil, fmt.Errorf("Index metadata.name expected *ves.io.schema.user.Object, got %T: %#v", e, e)
			}
		...
		"spec.gc_spec.email": store.NewIndexInfo(store.WithSecondaryIndex(func(e store.Entry) ([]string, error) {
			obj, ok := e.(*Object)
			if !ok {
				return nil, fmt.Errorf("Index spec.gc_spec.email expected *ves.io.schema.user.Object, got %T: %#v", e, e)
			}
		...
		"system_metadata.tenant": store.NewIndexInfo(store.WithSecondaryIndex(func(e store.Entry) ([]string, error) {
			obj, ok := e.(*Object)
			if !ok {
				return nil, fmt.Errorf("Index system_metadata.tenant expected *ves.io.schema.user.Object, got %T: %#v", e, e)
			}
			return []string{obj.GetSystemMetadata().GetTenant()}, nil
		})),
	}
	return indexers
...
func (e *DBObject) GetDRefInfo() ([]db.DRefInfo, error) {
	...
	if fdrInfos, err := e.GetSpecDRefInfo(); err != nil {
		...
		for i := range fdrInfos {
			...
			dri.DRField = "ves.io.schema.user.Object." + dri.DRField
			dri.RefrType = e.Type()
			dri.RefrUID = refrUID
...
func FindObjectStatus(ctx context.Context, d db.Interface, objUid string) ([]*StatusObject, error) {
	statusDBEntries, err := d.GetEntryBackrefs(ctx, objUid, ObjectType, db.WithBackrefTypes([]string{"ves.io.schema.user.StatusObject"}))
	if err != nil {
		return nil, err
	}
...
func (e *StatusObject) Type() string {
	return "ves.io.schema.user.StatusObject"
}
...
func (e *DBStatusObject) GetDRefInfo() ([]db.DRefInfo, error) {
	...
	for i := range drInfos {
		dri := &drInfos[i]
		// Convert Spec.LcSpec.vnRefs to ves.io.examplesvc.objectone.Object.Spec.LcSpec.vnRefs
		dri.DRField = "ves.io.schema.user.StatusObject." + dri.DRField
		dri.RefrType = e.Type()
		dri.RefrUID = refrUID
