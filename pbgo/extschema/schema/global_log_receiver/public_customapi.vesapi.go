...
func (s *customAPISrv) TestGlobalLogReceiver(ctx context.Context, in *TestGlobalLogReceiverRequest) (*TestGlobalLogReceiverResponse, error) {
	ah := s.svc.GetAPIHandler("ves.io.schema.global_log_receiver.CustomAPI")
	...
	bodyFields := svcfw.GenAuditReqBodyFields(ctx, s.svc, "ves.io.schema.global_log_receiver.TestGlobalLogReceiverRequest", in)
	defer func() {
		if len(bodyFields) > 0 {
	...
	if s.svc.Config().EnableAPIValidation {
		if rvFn := s.svc.GetRPCValidator("ves.io.schema.global_log_receiver.CustomAPI.TestGlobalLogReceiver"); rvFn != nil {
