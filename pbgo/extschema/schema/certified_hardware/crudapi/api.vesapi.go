...
import (
	...
	object "gopkg.volterra.us/terraform-provider-volterra/pbgo/extschema/schema/certified_hardware"
)
...
func (c *APIInprocClient) Create(ctx context.Context, req *ObjectCreateReq, opts ...grpc.CallOption) (*ObjectCreateRsp, error) {
	ah := c.crudCl.svc.GetAPIHandler("ves.io.schema.certified_hardware.crudapi.API")
	oah, ok := ah.(*APISrv)
	if !ok {
...
func (c *crudAPIInprocClient) Replace(ctx context.Context, e db.Entry, opts ...server.CRUDCallOpt) error {
	...
	oah, ok := ah.(*APISrv)
	if !ok {
		return fmt.Errorf("No CRUD Server for ves.io.schema.certified_hardware.crudapi")
	}
