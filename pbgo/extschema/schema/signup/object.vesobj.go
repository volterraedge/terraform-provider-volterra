...
const (
	ObjectDefTblName = "ves.io.schema.signup.Object.default"
	ObjectType       = "ves.io.schema.signup.Object"

	StatusObjectDefTblName = "ves.io.schema.signup.StatusObject.default"
	StatusObjectType       = "ves.io.schema.signup.StatusObject"
)

// augmented methods on protoc/std generated struct
func (e *Object) Type() string {
	return "ves.io.schema.signup.Object"
}
...
func GetObjectIndexers() store.Indexers {
	indexers := store.Indexers{
		"spec.gc_spec.email": store.NewIndexInfo(store.WithSecondaryIndex(func(e store.Entry) ([]string, error) {
			obj, ok := e.(*Object)
			if !ok {
				return nil, fmt.Errorf("Index spec.gc_spec.email expected *ves.io.schema.signup.Object, got %T: %#v", e, e)
			}
...
func (e *DBObject) GetDRefInfo() ([]db.DRefInfo, error) {
	...
	if fdrInfos, err := e.GetSpecDRefInfo(); err != nil {
		...
		for i := range fdrInfos {
			...
			dri.DRField = "ves.io.schema.signup.Object." + dri.DRField
			dri.RefrType = e.Type()
			dri.RefrUID = refrUID
...
func FindObjectStatus(ctx context.Context, d db.Interface, objUid string) ([]*StatusObject, error) {
	statusDBEntries, err := d.GetEntryBackrefs(ctx, objUid, ObjectType, db.WithBackrefTypes([]string{"ves.io.schema.signup.StatusObject"}))
	if err != nil {
		return nil, err
	}
...
func (e *StatusObject) Type() string {
	return "ves.io.schema.signup.StatusObject"
}
...
func (e *DBStatusObject) GetDRefInfo() ([]db.DRefInfo, error) {
	...
	if fdrInfos, err := e.GetObjectRefsDRefInfo(); err != nil {
		...
		for i := range fdrInfos {
			dri := &fdrInfos[i]
			// Convert Spec.LcSpec.vnRefs to ves.io.examplesvc.objectone.Object.Spec.LcSpec.vnRefs
			dri.DRField = "ves.io.schema.signup.StatusObject." + dri.DRField
			dri.RefrType = e.Type()
			dri.RefrUID = refrUID
	...
	if fdrInfos, err := e.GetUserDRefInfo(); err != nil {
...
func (e *DBStatusObject) GetObjectRefsDBEntries(ctx context.Context, d db.Interface) ([]db.Entry, error) {
	...
	refdType, err := d.TypeForEntryKind(e.Type(), refrUID, "signup.Object")
	if err != nil {
...
func (e *DBStatusObject) GetUserDRefInfo() ([]db.DRefInfo, error) {
	refrUID, err := e.Key()
	if err != nil {
		return nil, errors.Wrap(err, "GetDRefInfo, error in key")
	}
	...
	for i, ref := range refs {
		if ref == nil {
			return nil, fmt.Errorf("StatusObject.user[%d] has a nil value", i)
		}
		// resolve kind to type if needed at DBObject.GetDRefInfo()
		drInfos = append(drInfos, db.DRefInfo{
...
func (e *DBStatusObject) GetUserDBEntries(ctx context.Context, d db.Interface) ([]db.Entry, error) {
	var entries []db.Entry
	refrUID, err := e.Key()
	if err != nil {
		return nil, errors.Wrap(err, "Get<fld>DBEntries, error in key")
	}
	refdType, err := d.TypeForEntryKind(e.Type(), refrUID, "user.Object")
	if err != nil {
...
func (v *ValidateStatusObject) Validate(ctx context.Context, pm interface{}, opts ...db.ValidateOpt) error {
	...
	if fv, exists := v.FldValidators["billingAccount"]; exists {

		vOpts := append(opts, db.WithValidateField("billingAccount"))
		for idx, item := range e.GetBillingAccount() {
	...
	if fv, exists := v.FldValidators["tenant"]; exists {
		...
		for idx, item := range e.GetTenant() {
			vOpts := append(vOpts, db.WithValidateRepItem(idx), db.WithValidateIsRepItem(true))
			if err := fv(ctx, item, vOpts...); err != nil {
				return err
			}
