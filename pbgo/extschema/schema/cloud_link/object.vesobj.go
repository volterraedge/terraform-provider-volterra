...
const (
	ObjectDefTblName = "ves.io.schema.cloud_link.Object.default"
	ObjectType       = "ves.io.schema.cloud_link.Object"

	StatusObjectDefTblName = "ves.io.schema.cloud_link.StatusObject.default"
	StatusObjectType       = "ves.io.schema.cloud_link.StatusObject"
)

// augmented methods on protoc/std generated struct
func (e *Object) Type() string {
	return "ves.io.schema.cloud_link.Object"
}
...
func (e *DBObject) GetDRefInfo() ([]db.DRefInfo, error) {
	...
	if fdrInfos, err := e.GetSpecDRefInfo(); err != nil {
		...
		for i := range fdrInfos {
			...
			dri.DRField = "ves.io.schema.cloud_link.Object." + dri.DRField
			dri.RefrType = e.Type()
			dri.RefrUID = refrUID
...
func FindObjectStatus(ctx context.Context, d db.Interface, objUid string) ([]*StatusObject, error) {
	statusDBEntries, err := d.GetEntryBackrefs(ctx, objUid, ObjectType, db.WithBackrefTypes([]string{"ves.io.schema.cloud_link.StatusObject"}))
	if err != nil {
		return nil, err
	}
...
func (e *StatusObject) Type() string {
	return "ves.io.schema.cloud_link.StatusObject"
}
...
func (e *DBStatusObject) GetDRefInfo() ([]db.DRefInfo, error) {
	...
	for i := range drInfos {
		dri := &drInfos[i]
		// Convert Spec.LcSpec.vnRefs to ves.io.examplesvc.objectone.Object.Spec.LcSpec.vnRefs
		dri.DRField = "ves.io.schema.cloud_link.StatusObject." + dri.DRField
		dri.RefrType = e.Type()
		dri.RefrUID = refrUID
...
func (e *DBStatusObject) GetObjectRefsDBEntries(ctx context.Context, d db.Interface) ([]db.Entry, error) {
	...
	refdType, err := d.TypeForEntryKind(e.Type(), refrUID, "cloud_link.Object")
	if err != nil {
...
func (v *ValidateStatusObject) Validate(ctx context.Context, pm interface{}, opts ...db.ValidateOpt) error {
	...
	switch e.GetCloudLinkStatus().(type) {
	case *StatusObject_AwsStatus:
		if fv, exists := v.FldValidators["cloud_link_status.aws_status"]; exists {
			val := e.GetCloudLinkStatus().(*StatusObject_AwsStatus).AwsStatus
			vOpts := append(opts,
				db.WithValidateField("cloud_link_status"),
				db.WithValidateField("aws_status"),
			)
			if err := fv(ctx, val, vOpts...); err != nil {
				return err
			}
		}
	case *StatusObject_AzureStatus:
		if fv, exists := v.FldValidators["cloud_link_status.azure_status"]; exists {
			val := e.GetCloudLinkStatus().(*StatusObject_AzureStatus).AzureStatus
			vOpts := append(opts,
				db.WithValidateField("cloud_link_status"),
				db.WithValidateField("azure_status"),
			)
			if err := fv(ctx, val, vOpts...); err != nil {
				return err
			}
