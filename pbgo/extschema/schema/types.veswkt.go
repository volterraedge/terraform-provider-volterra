...
// GetNamespaceRef returns values of tenant, namespace, and namespaceUID an object is present in
func (m *SystemObjectMetaType) GetNamespaceRef() (string, string, string) {
	if m == nil || m.Namespace == nil {
		return "", "", ""
	}
	return m.Namespace[0].Tenant, m.Namespace[0].Name, m.Namespace[0].Uid
}

// SetNamespaceRef creates a reference to namespace object
func (m *SystemObjectMetaType) SetNamespaceRef(tenant, nsName, nsUID string) error {
	m.Namespace = []*ObjectRefType{
		{Kind: "namespace", Tenant: tenant, Name: nsName, Uid: nsUID},
	}
	return nil
}
