...
func (c *customDataAPIInprocClient) Usage(ctx context.Context, in *UsageRequest, opts ...grpc.CallOption) (*UsageResponse, error) {
	ctx = server.ContextWithRpcFQN(ctx, "ves.io.schema.views.workload.CustomDataAPI.Usage")
	return c.CustomDataAPIServer.Usage(ctx, in)
}
...
func (s *customDataAPISrv) Usage(ctx context.Context, in *UsageRequest) (*UsageResponse, error) {
	ah := s.svc.GetAPIHandler("ves.io.schema.views.workload.CustomDataAPI")
	...
	bodyFields := svcfw.GenAuditReqBodyFields(ctx, s.svc, "ves.io.schema.views.workload.UsageRequest", in)
	defer func() {
		if len(bodyFields) > 0 {
	...
	if s.svc.Config().EnableAPIValidation {
		if rvFn := s.svc.GetRPCValidator("ves.io.schema.views.workload.CustomDataAPI.Usage"); rvFn != nil {
