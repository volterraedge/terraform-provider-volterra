...
func (c *crudAPIRestClient) Create(ctx context.Context, e db.Entry, opts ...server.CRUDCallOpt) (db.Entry, error) {
	...
	url := fmt.Sprintf("%s/public/namespaces/%s/waf_ruless", c.baseURL, req.Metadata.GetNamespace())
	jsn, err := req.ToJSON()
	if err != nil {
...
func (c *crudAPIRestClient) Replace(ctx context.Context, e db.Entry, opts ...server.CRUDCallOpt) error {
	...
	url := fmt.Sprintf("%s/public/namespaces/%s/waf_ruless/%s", c.baseURL, namespace, name)
...
func (c *crudAPIRestClient) GetRaw(ctx context.Context, key string, opts ...server.CRUDCallOpt) (*GetResponse, error) {
	...
	url := fmt.Sprintf("%s/public/namespaces/%s/waf_ruless/%s", c.baseURL, req.Namespace, req.Name)
...
func (c *crudAPIRestClient) List(ctx context.Context, opts ...server.CRUDCallOpt) (*ListResponse, error) {
	req := NewListRequest(opts...)
	_ = req
	url := fmt.Sprintf("%s/public/namespaces/%s/waf_ruless", c.baseURL, req.Namespace)
...
func (c *crudAPIRestClient) Delete(ctx context.Context, key string, opts ...server.CRUDCallOpt) error {
	...
	url := fmt.Sprintf("%s/public/namespaces/%s/waf_ruless/%s", c.baseURL, dReq.Namespace, dReq.Name)
...
func (c *APIInprocClient) Create(ctx context.Context, req *CreateRequest, opts ...grpc.CallOption) (*CreateResponse, error) {
	ah := c.crudCl.svc.GetAPIHandler("ves.io.schema.waf_rules.API")
	oah, ok := ah.(*APISrv)
	if !ok {
		err := fmt.Errorf("No CRUD Server for ves.io.schema.waf_rules")
...
func (c *crudAPIInprocClient) Create(ctx context.Context, e db.Entry, opts ...server.CRUDCallOpt) (db.Entry, error) {
	...
	ah := c.svc.GetAPIHandler("ves.io.schema.waf_rules.API")
	oah, ok := ah.(*APISrv)
	if !ok {
		return nil, fmt.Errorf("No CRUD Server for ves.io.schema.waf_rules")
	}
...
func (c *crudAPIInprocClient) Replace(ctx context.Context, e db.Entry, opts ...server.CRUDCallOpt) error {
	...
	oah, ok := ah.(*APISrv)
	if !ok {
		return fmt.Errorf("No CRUD Server for ves.io.schema.waf_rules")
	}
...
func (s *APISrv) validateTransport(ctx context.Context) error {
	if s.sf.IsTransportNotSupported("ves.io.schema.waf_rules.API", server.TransportFromContext(ctx)) {
		userMsg := fmt.Sprintf("ves.io.schema.waf_rules.API not allowed in transport '%s'", server.TransportFromContext(ctx))
...
func (s *APISrv) Create(ctx context.Context, req *CreateRequest) (*CreateResponse, error) {
	...
	if s.sf.Config().EnableAPIValidation {
		if rvFn := s.sf.GetRPCValidator("ves.io.schema.waf_rules.API.Create"); rvFn != nil {
			if err := rvFn(ctx, req); err != nil {
				if !server.NoReqValidateFromContext(ctx) {
					err := server.MaybePublicRestError(ctx, errors.Wrapf(err, "Validating Request"))
					return nil, server.GRPCStatusFromError(err).Err()
				}
				s.sf.Logger().Warn(server.NoReqValidateAcceptLog, zap.String("rpc_fqn", "ves.io.schema.waf_rules.API.Create"), zap.Error(err))
			}
		}
	}
	reqMsgFQN := "ves.io.schema.waf_rules.CreateRequest"
	...
	rspMsgFQN := "ves.io.schema.waf_rules.CreateResponse"
	if conv, exists := s.sf.Config().ObjToMsgConverters[rspMsgFQN]; exists {
		if err := conv(rsrcRsp.Entry, rsp); err != nil {
			return nil, err
		}
	}
	bodyFields = append(bodyFields, svcfw.GenAuditRspBodyFields(ctx, s.sf, "ves.io.schema.waf_rules.API.CreateResponse", rsp)...)
	return rsp, nil
}
...
func (s *APISrv) Replace(ctx context.Context, req *ReplaceRequest) (*ReplaceResponse, error) {
	...
	if s.sf.Config().EnableAPIValidation {
		if rvFn := s.sf.GetRPCValidator("ves.io.schema.waf_rules.API.Replace"); rvFn != nil {
			if err := rvFn(ctx, req); err != nil {
				if !server.NoReqValidateFromContext(ctx) {
					err := server.MaybePublicRestError(ctx, errors.Wrapf(err, "Validating Request"))
					return nil, server.GRPCStatusFromError(err).Err()
				}
				s.sf.Logger().Warn(server.NoReqValidateAcceptLog, zap.String("rpc_fqn", "ves.io.schema.waf_rules.API.Replace"), zap.Error(err))
			}
		}
	}
	bodyFields := svcfw.GenAuditReqBodyFields(ctx, s.sf, "ves.io.schema.waf_rules.API.ReplaceRequest", req)
	defer func() {
		if len(bodyFields) > 0 {
			server.ExtendAPIAudit(ctx, svcfw.PublicAPIBodyLog.Uid, bodyFields)
		}
	...
	bodyFields = append(bodyFields, svcfw.GenAuditRspBodyFields(ctx, s.sf, "ves.io.schema.waf_rules.API.ReplaceResponse", rsp)...)
	return rsp, nil
}
...
func (s *APISrv) Get(ctx context.Context, req *GetRequest) (*GetResponse, error) {
	...
	if s.sf.Config().EnableAPIValidation {
		if rvFn := s.sf.GetRPCValidator("ves.io.schema.waf_rules.API.Get"); rvFn != nil {
			if err := rvFn(ctx, req); err != nil {
				err := server.MaybePublicRestError(ctx, errors.Wrapf(err, "Validating Request"))
				return nil, server.GRPCStatusFromError(err).Err()
			}
	...
	rspMsgFQN := "ves.io.schema.waf_rules.GetResponse"
	if conv, exists := s.sf.Config().ObjToMsgConverters[rspMsgFQN]; exists {
		if err := conv(rsrcRsp.Entry, rsp); err != nil {
			return nil, err
		}
...
func (s *APISrv) List(ctx context.Context, req *ListRequest) (*ListResponse, error) {
	...
	if s.sf.Config().EnableAPIValidation {
		if rvFn := s.sf.GetRPCValidator("ves.io.schema.waf_rules.API.List"); rvFn != nil {
			if err := rvFn(ctx, req); err != nil {
				err := server.MaybePublicRestError(ctx, errors.Wrapf(err, "Validating Request"))
				return nil, server.GRPCStatusFromError(err).Err()
			}
...
func (s *APISrv) Delete(ctx context.Context, req *DeleteRequest) (*google_protobuf.Empty, error) {
	...
	if s.sf.Config().EnableAPIValidation {
		if rvFn := s.sf.GetRPCValidator("ves.io.schema.waf_rules.API.Delete"); rvFn != nil {
			if err := rvFn(ctx, req); err != nil {
				if !server.NoReqValidateFromContext(ctx) {
					err := server.MaybePublicRestError(ctx, errors.Wrapf(err, "Validating Request"))
					return nil, server.GRPCStatusFromError(err).Err()
				}
				s.sf.Logger().Warn(server.NoReqValidateAcceptLog, zap.String("rpc_fqn", "ves.io.schema.waf_rules.API.Delete"), zap.Error(err))
			}
		}
	}
	bodyFields := svcfw.GenAuditReqBodyFields(ctx, s.sf, "ves.io.schema.waf_rules.API.DeleteRequest", req)
	defer func() {
		if len(bodyFields) > 0 {
			server.ExtendAPIAudit(ctx, svcfw.PublicAPIBodyLog.Uid, bodyFields)
		}
	...
	for _, rsrcItem := range rsrcRsp.Items {
		...
		if len(req.ReportFields) > 0 {
			...
			if o.Object != nil && o.Object.GetSpec().GetGcSpec() != nil {
				msgFQN := "ves.io.schema.waf_rules.GetResponse"
...
var APISwaggerJSON string = `{
    ...
    "tags": null,
    "paths": {
        "/public/namespaces/{metadata.namespace}/waf_ruless": {
            "post": {
                "summary": "Create WAF Rules",
                "description": "Create App type will create the configuration in namespace metadata.namespace",
                "operationId": "ves.io.schema.waf_rules.API.Create",
                "responses": {
                    "200": {
                        "description": "A successful response.",
                        "schema": {
                            "$ref": "#/definitions/waf_rulesCreateResponse"
                        }
                    },
                    "401": {
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/waf_rulesCreateRequest"
                        }
                    }
                ],
                "tags": [
                ...
                "externalDocs": {
                    ...
                    "url": "https://www.volterra.io/docs/reference/api-ref/ves-io-schema-waf_rules-API-Create"
                },
                "x-ves-proto-rpc": "ves.io.schema.waf_rules.API.Create"
            },
            "x-displayname": "WAF Rules",
            "x-ves-proto-service": "ves.io.schema.waf_rules.API",
            "x-ves-proto-service-type": "AUTO_CRUD_PUBLIC"
        },
        "/public/namespaces/{metadata.namespace}/waf_ruless/{metadata.name}": {
            "put": {
                "summary": "Replace WAF Rules",
                "description": "Update the configuration by replacing the existing spec with the provided one. \nFor read-then-write operations a resourceVersion mismatch will occur if the object was modified between the read and write.",
                "operationId": "ves.io.schema.waf_rules.API.Replace",
                "responses": {
                    "200": {
                        "description": "A successful response.",
                        "schema": {
                            "$ref": "#/definitions/waf_rulesReplaceResponse"
                        }
                    },
                    "401": {
                        "description": "Returned when operation is not authorized",
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/waf_rulesReplaceRequest"
                        }
                    }
                ],
                "tags": [
                ...
                "externalDocs": {
                    "description": "Examples of this operation",
                    "url": "https://www.volterra.io/docs/reference/api-ref/ves-io-schema-waf_rules-API-Replace"
                },
                "x-ves-proto-rpc": "ves.io.schema.waf_rules.API.Replace"
            },
            "x-displayname": "WAF Rules",
            "x-ves-proto-service": "ves.io.schema.waf_rules.API",
            "x-ves-proto-service-type": "AUTO_CRUD_PUBLIC"
        },
        "/public/namespaces/{namespace}/waf_ruless": {
            "get": {
                "summary": "List",
                "description": "List the set of waf_rules in a namespace",
                "operationId": "ves.io.schema.waf_rules.API.List",
                "responses": {
                    "200": {
                        "description": "A successful response.",
                        "schema": {
                            "$ref": "#/definitions/waf_rulesListResponse"
                        }
                    },
                    "401": {
                        "description": "Returned when operation is not authorized",
                ...
                "parameters": [
                    {
                        "name": "namespace",
                        "description": "namespace\n\nx-example: \"ns1\"\nNamespace to scope the listing of waf_rules",
                        "in": "path",
                        "required": true,
                        "type": "string",
                        "x-displayname": "Namespace"
                ...
                "externalDocs": {
                    "description": "Examples of this operation",
                    "url": "https://www.volterra.io/docs/reference/api-ref/ves-io-schema-waf_rules-API-List"
                },
                "x-ves-proto-rpc": "ves.io.schema.waf_rules.API.List"
            },
            "x-displayname": "WAF Rules",
            "x-ves-proto-service": "ves.io.schema.waf_rules.API",
            "x-ves-proto-service-type": "AUTO_CRUD_PUBLIC"
        },
        "/public/namespaces/{namespace}/waf_ruless/{name}": {
            "get": {
                "summary": "Get WAF Rules",
                "description": "Get App type will read the configuration from namespace metadata.namespace",
                "operationId": "ves.io.schema.waf_rules.API.Get",
                "responses": {
                    "200": {
                        "description": "A successful response.",
                        "schema": {
                            "$ref": "#/definitions/waf_rulesGetResponse"
                        }
                    },
                    "401": {
                        "description": "Returned when operation is not authorized",
                ...
                "externalDocs": {
                    "description": "Examples of this operation",
                    "url": "https://www.volterra.io/docs/reference/api-ref/ves-io-schema-waf_rules-API-Get"
                },
                "x-ves-proto-rpc": "ves.io.schema.waf_rules.API.Get"
            },
            "delete": {
                "summary": "Delete",
                "description": "Delete the specified waf_rules",
                "operationId": "ves.io.schema.waf_rules.API.Delete",
                "responses": {
                    "200": {
                        "description": "A successful response.",
                        "schema": {}
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/waf_rulesDeleteRequest"
                        }
                    }
                ],
                "tags": [
                ...
                "externalDocs": {
                    "description": "Examples of this operation",
                    "url": "https://www.volterra.io/docs/reference/api-ref/ves-io-schema-waf_rules-API-Delete"
                },
                "x-ves-proto-rpc": "ves.io.schema.waf_rules.API.Delete"
            },
            "x-displayname": "WAF Rules",
            "x-ves-proto-service": "ves.io.schema.waf_rules.API",
            "x-ves-proto-service-type": "AUTO_CRUD_PUBLIC"
    ...
    "definitions": {
        ...
        "schemawaf_rulesCreateSpecType": {
            "type": "object",
            "description": "Create App type will create the configuration in namespace metadata.namespace",
            "title": "Create WAF Rules",
            "x-displayname": "Create WAF Rules",
            "x-ves-proto-message": "ves.io.schema.waf_rules.CreateSpecType",
            "properties": {
                ...
                "rule_ids": {
                    "type": "array",
                    "description": " rule IDs to be included or excluded in this WAF instance\n\nExample: - \"[941210, 941310]\"-",
                    "items": {
                        "$ref": "#/definitions/waf_rule_listWafRuleID"
                    },
                    "x-displayname": "Rule IDs",
                    "x-ves-example": "[941210, 941310]"
                },
                "rule_list_type": {
                    "description": " Specify whether the defined rule_ids list is to be included or excluded from associated WAF instance\nRequired: YES",
        ...
        "schemawaf_rulesGetSpecType": {
            "type": "object",
            "description": "Get App type will read the configuration from namespace metadata.namespace",
            "title": "Get WAF Rules",
            "x-displayname": "Get WAF Rules",
            "x-ves-proto-message": "ves.io.schema.waf_rules.GetSpecType",
            "properties": {
                "anomaly_score_threshold": {
                    "type": "integer",
                    "description": " If anomaly score is same or above threshold , the req/resp will be blocked/alerted (depending\n on non_blocking_mode configuration)\n\nExample: - \"4\"-\nRequired: YES",
        ...
        "schemawaf_rulesGlobalSpecType": {
            "type": "object",
            "title": "GlobalSpecType",
            "x-displayname": "Specification",
            "x-ves-proto-message": "ves.io.schema.waf_rules.GlobalSpecType",
            "properties": {
                "anomaly_score_threshold": {
                    "type": "integer",
                    "description": " If anomaly score is same or above threshold , the req/resp will be blocked/alerted (depending\n on non_blocking_mode configuration)\n\nExample: - \"4\"-\nRequired: YES",
                ...
                "waf": {
                    "type": "array",
                    "description": " A reference to WAF object",
                    "title": "waf",
                    "items": {
                        "$ref": "#/definitions/schemaObjectRefType"
                    },
        ...
        "schemawaf_rulesReplaceSpecType": {
            "type": "object",
            "description": "Update the configuration by replacing the existing spec with the provided one. \nFor read-then-write operations a resourceVersion mismatch will occur if the object was modified between the read and write.",
            "title": "Replace WAF Rules",
            "x-displayname": "Replace WAF Rules",
            "x-ves-proto-message": "ves.io.schema.waf_rules.ReplaceSpecType",
            "properties": {
                "anomaly_score_threshold": {
                    "type": "integer",
                    "description": " If anomaly score is same or above threshold , the req/resp will be blocked/alerted (depending\n on non_blocking_mode configuration)\n\nExample: - \"4\"-\nRequired: YES",
        ...
        "waf_rulesCreateRequest": {
            "type": "object",
            "description": "This is the input message of the 'Create' RPC",
            "title": "CreateRequest is used to create an instance of waf_rules",
            "x-displayname": "Create Request",
            "x-ves-proto-message": "ves.io.schema.waf_rules.CreateRequest",
            "properties": {
                "metadata": {
                ...
                "spec": {
                    ...
                    "title": "spec",
                    "$ref": "#/definitions/schemawaf_rulesCreateSpecType",
                    "x-displayname": "Spec"
                }
            }
        },
        "waf_rulesCreateResponse": {
            "type": "object",
            "x-ves-proto-message": "ves.io.schema.waf_rules.CreateResponse",
            "properties": {
                "metadata": {
                    "description": " Standard object's metadata",
                    "title": "metadata",
                ...
                "spec": {
                    "description": " Specification of the desired behavior of the WAF instance",
                    "title": "spec",
                    "$ref": "#/definitions/schemawaf_rulesGetSpecType",
                    "x-displayname": "Spec"
                },
                "system_metadata": {
        ...
        "waf_rulesDeleteRequest": {
            "type": "object",
            "description": "This is the input message of the 'Delete' RPC.",
            "title": "DeleteRequest is used to delete a waf_rules",
            "x-displayname": "Delete Request",
            "x-ves-proto-message": "ves.io.schema.waf_rules.DeleteRequest",
            "properties": {
                "fail_if_referred": {
                    "type": "boolean",
        ...
        "waf_rulesGetResponse": {
            "type": "object",
            "description": "This is the output message of the 'Get' RPC",
            "title": "GetResponse is the shape of a read waf_rules",
            "x-displayname": "Get Response",
            "x-ves-proto-message": "ves.io.schema.waf_rules.GetResponse",
            "properties": {
                "create_form": {
                ...
                "status": {
                    ...
                    "title": "status",
                    "items": {
                        "$ref": "#/definitions/waf_rulesStatusObject"
                    },
                    "x-displayname": "Status"
                },
                "system_metadata": {
        ...
        "waf_rulesListResponse": {
            "type": "object",
            "description": "This is the output message of 'List' RPC.",
            "title": "ListResponse is the collection of waf_rules",
            "x-displayname": "List Response",
            "x-ves-proto-message": "ves.io.schema.waf_rules.ListResponse",
            "properties": {
                "errors": {
                    "type": "array",
                ...
                "items": {
                    ...
                    "title": "items",
                    "items": {
                        "$ref": "#/definitions/waf_rulesListResponseItem"
                    },
                    "x-displayname": "Items"
                }
            }
        },
        "waf_rulesListResponseItem": {
            "type": "object",
            "description": "By default a summary of waf_rules is returned in 'List'. By setting\n'report_fields' in the ListRequest more details of each item can be got.",
            "title": "ListResponseItem is an individual item in a collection of waf_rules",
            "x-displayname": "List Item",
            "x-ves-proto-message": "ves.io.schema.waf_rules.ListResponseItem",
            "properties": {
                "annotations": {
                    "type": "object",
                    "description": " The set of annotations present on this waf_rules",
                    "title": "annotations",
                    "x-displayname": "Annotations"
                },
                "description": {
                    "type": "string",
                    "description": " The description set for this waf_rules",
                    "title": "description",
                    "x-displayname": "Description"
                },
                "disabled": {
                    "type": "boolean",
                    "description": " A value of true indicates waf_rules is administratively disabled",
                    "title": "disabled",
                    "format": "boolean",
                ...
                "labels": {
                    "type": "object",
                    "description": " The set of labels present on this waf_rules",
                    "title": "labels",
                    "x-displayname": "Labels"
                },
                "metadata": {
                ...
                "name": {
                    "type": "string",
                    "description": " The name of this waf_rules\n\nExample: - \"name\"-",
                    "title": "name",
                    "x-displayname": "Name",
                    "x-ves-example": "name"
                },
                ...
                "uid": {
                    "type": "string",
                    "description": " The unique uid of this waf_rules\n\nExample: - \"d27938ba-967e-40a7-9709-57b8627f9f75\"-",
                    "title": "uid",
                    "x-displayname": "UID",
        ...
        "waf_rulesObject": {
            "type": "object",
            "description": "WAF Rules Object to define low level detailed configuration for a WAF instance which can be associated with vhosts or routes.",
            "title": "WAF Rules Object",
            "x-displayname": "WAF Rules Object",
            "x-ves-proto-message": "ves.io.schema.waf_rules.Object",
            "properties": {
                "metadata": {
                    "description": " Standard object's metadata",
                    "title": "metadata",
                ...
                "spec": {
                    "description": " Specification of the desired behavior of the WAF instance",
                    "title": "spec",
                    "$ref": "#/definitions/waf_rulesSpecType",
                    "x-displayname": "Spec"
                },
                "system_metadata": {
                    "description": " System generated object's metadata",
        ...
        "waf_rulesReplaceRequest": {
            "type": "object",
            "description": "This is the input message of the 'Replace' RPC",
            "title": "ReplaceRequest is used to replace contents of a waf_rules",
            "x-displayname": "Replace Request",
            "x-ves-proto-message": "ves.io.schema.waf_rules.ReplaceRequest",
            "properties": {
                "metadata": {
                    "description": " Standard object's metadata",
                    "title": "metadata",
                ...
                "spec": {
                    "description": " Specification of the desired behavior of the WAF instance",
                    "title": "spec",
                    "$ref": "#/definitions/schemawaf_rulesReplaceSpecType",
                    "x-displayname": "Spec"
                }
            }
        },
        "waf_rulesReplaceResponse": {
            "type": "object",
            "x-ves-proto-message": "ves.io.schema.waf_rules.ReplaceResponse"
        },
        ...
        "waf_rulesSpecType": {
            ...
            "title": "WAF Rules Specification",
            "x-displayname": "Specification",
            "x-ves-proto-message": "ves.io.schema.waf_rules.SpecType",
            "properties": {
                "gc_spec": {
                    "title": "gc_spec",
                    "$ref": "#/definitions/schemawaf_rulesGlobalSpecType",
                    "x-displayname": "GC Spec"
                }
            }
        },
        "waf_rulesStatusObject": {
            "type": "object",
    ...
    "x-displayname": "WAF Rules",
    "x-ves-proto-file": "ves.io/schema/waf_rules/public_crudapi.proto"
}`
