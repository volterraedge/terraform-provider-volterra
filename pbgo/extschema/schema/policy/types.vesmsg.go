...
func (v *ValidateShapeProtectedEndpointAction) GoodBotValidationRuleHandler(rules map[string]string) (db.ValidatorFunc, error) {

	itemRules := db.GetRepStringItemRules(rules)
	itemValFn, err := db.NewStringValidationRuleHandler(itemRules)
	if err != nil {
		return nil, errors.Wrap(err, "Item ValidationRuleHandler for good_bot")
	}
	itemsValidatorFn := func(ctx context.Context, elems []string, opts ...db.ValidateOpt) error {
		for i, el := range elems {
			if err := itemValFn(ctx, el, opts...); err != nil {
	...
	repValFn, err := db.NewRepeatedValidationRuleHandler(rules)
	if err != nil {
		return nil, errors.Wrap(err, "Repeated ValidationRuleHandler for good_bot")
	}
	...
	validatorFn := func(ctx context.Context, val interface{}, opts ...db.ValidateOpt) error {
		...
		if err := repValFn(ctx, l, opts...); err != nil {
			return errors.Wrap(err, "repeated good_bot")
		}
		if err := itemsValidatorFn(ctx, elems, opts...); err != nil {
			return errors.Wrap(err, "items good_bot")
		}
		return nil
	}
...
func (v *ValidateShapeProtectedEndpointAction) Validate(ctx context.Context, pm interface{}, opts ...db.ValidateOpt) error {
	...
	if fv, exists := v.FldValidators["good_bot"]; exists {
		vOpts := append(opts, db.WithValidateField("good_bot"))
		if err := fv(ctx, m.GetGoodBot(), vOpts...); err != nil {
			return err
		}
...
var DefaultShapeProtectedEndpointActionValidator = func() *ValidateShapeProtectedEndpointAction {
	...
	vrhGoodBot := v.GoodBotValidationRuleHandler
	rulesGoodBot := map[string]string{
		"ves.io.schema.rules.message.required":              "true",
		"ves.io.schema.rules.repeated.items.string.max_len": "128",
		"ves.io.schema.rules.repeated.max_items":            "256",
	}
	vFn, err = vrhGoodBot(rulesGoodBot)
	if err != nil {
		errMsg := fmt.Sprintf("ValidationRuleHandler for ShapeProtectedEndpointAction.good_bot: %s", err)
		panic(errMsg)
	}
	v.FldValidators["good_bot"] = vFn
